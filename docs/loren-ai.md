...
- **💬 Conversational Memory**: Remembers context within a session. "More like that", "the third one", "what was that book again?" — all work because Loren tracks the last results, genre, length, and last mentioned book.
- **🏷 Inline Book Actions**: Every result card shown in the chat has three action buttons: Open (launch reader), Save (toggle favorite), Mark read (toggle read status) — without leaving the conversation.
